<template>
  <div>
      ...
      <v-card>
        ...
        <v-card-text>
          <v-container grid-list-md>
            <v-layout wrap>
              <v-flex xs5>
                <v-select v-if="tipoVeiculo" v-bind:items="tipoVeiculo" label="Tipo de Veiculo" item-text="nome" item-value="id" v-model="formVeiculo.id_tipo_veiculo" name="id_tipo_veiculo"></v-select>
                <show-error :form-name="formVeiculo" prop-name="id_tipo_veiculo"></show-error>
              </v-flex>
              <v-flex xs5>
                <v-autocomplete v-if="cliente" v-bind:items="cliente" label="Cliente" item-text="nome" item-value="id" v-model="formVeiculo.id_cliente" name="id_cliente"></v-autocomplete>
    ...
    <v-card>
      ...
      <v-data-table :headers="headers" :items="veiculo" :search="search" rows-per-page-text="por página" no-results-text="Nenhum registro encontrado" :rows-per-page-items="rowsPerPageItems">
...
export default {
  ...
  data: () => ({
    ...
    veiculo: [],
    cliente: [],
    headers: [{ text: "Tipo de Veiculo", value: "id_tipo_veiculo" }, { text: "Modelo", value: "modelo" }, { text: "Cliente", value: "id_cliente" }, { text: "Ações", value: "", align: "center" }],
  ...
  methods: {
    ...
    getTipoVeiculo() {
      axios.get("/api/tipo-veiculo").then(res => {
    ...
    remover(id) {
      this.$swal({
        ...
        if (result.value) {
          axios.delete("/api/veiculo/" + id).then(response => {
            if (response.data.status == "success") {
              this.$swal.fire("Excluído!", "", "success");
              this.getVeiculo();
            } else {
              this.$swal.fire("Veiculo não foi excluído", "", "error");
            }
          });
        }
      });
    ...
    salvar() {
      ...
      if (this.formVeiculo.id) {
        axios
          .put("/api/veiculo/" + this.formVeiculo.id, this.formVeiculo)
          .then(response => {
            this.dialog = false;
            loading.hide();
