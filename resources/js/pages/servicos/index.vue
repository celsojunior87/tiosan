            ...
            <v-layout wrap>
              ...
              <v-flex xs4>
                <v-text-field
                  ...
                  v-model="formServico.hora_entrada"
                  return-masked-value
                  mask=" ##:##"
                  required
                ></v-text-field>
              ...
              <v-flex xs4>
                <v-text-field
                  label="Hora de Saida"
                  v-model="formServico.hora_saida"
                  return-masked-value
                  mask=" ##:##"
                  required
                ></v-text-field>
        ...
        <template v-slot:items="props">
          ...
          <td class="text-xs-center">
            <v-btn small color="blue" @click="editar(props.item.id)">
              <v-icon dark left>edit</v-icon>Editar Serviço
            </v-btn>
            <v-btn small color="green" @click="enviarEmail(props.item.id)">
              <v-icon dark left>send</v-icon>Finalizar Serviço
            </v-btn>
          </td>
        </template>
        <v-alert
...
export default {
  ...
  methods: {
    ...
    getTipoServico() {
      axios.get("/api/tipo-servico").then(res => {
    ...
    editar(id) {
      axios.get("/api/servico/" + id).then(res => {
        this.formServico = res.data;
        this.formServico.preco = parseFloat(res.data.preco).toFixed(2)      
         this.formServico.preco_adc = parseFloat(res.data.preco_adc).toFixed(2)    
    ...
    remover(id) {
      this.$swal({
        ...
        if (result.value) {
          axios.delete("/api/servico/" + id).then(response => {
            if (response.data.status == "success") {
              this.$swal.fire("Excluído!", "", "success");
              this.getServico();
            } else {
              this.$swal.fire("Serviço não foi excluído", "", "error");
            }
          });
        }
      });
    ...
    salvar() {
      ...
      if (this.formServico.id) {
        axios
          .put("/api/servico/" + this.formServico.id, this.formServico)
          .then(response => {
            this.dialog = false;
            loading.hide();
